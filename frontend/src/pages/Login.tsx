/* ============================================================
   VPC-HMS — Login Page
   Animated gradient background, glass card, role quick-access
   buttons. Username field enforces numeric-only input (mobile
   number format, 10 digits) at the keystroke level.
   ============================================================ */
...
export default function LoginPage() {
  ...
  // ── Numeric-only username handlers ────────────────────────

  /**
   * Block any non-numeric keystroke in real time.
   * Allowed: digits 0-9, navigation keys (Backspace, Delete, Arrow keys, Tab).
   */
  const handleUsernameKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (!/^\d$/.test(e.key)) {
      e.preventDefault();
      setUsernameError('Only numeric digits (0–9) are allowed — use your 10-digit mobile number');
    }
  };

  /**
   * Strip non-numeric characters from pasted content before applying to state.
   */
  const handleUsernamePaste = (e: React.ClipboardEvent<HTMLInputElement>) => {
    ...
    setUsername(numeric);
    if (pasted !== numeric) {
      setUsernameError('Non-numeric characters were removed — only digits allowed');
    } else {
      setUsernameError('');
    }
  };
  ...
  /**
   * onChange safety net: ensure only digits remain even if browser auto-fill
   * inserts non-numeric characters.
   */
  const handleUsernameChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (raw !== numeric) {
      setUsernameError('Only numeric digits allowed (0–9)');
    } else if (numeric.length > 0 && numeric.length < 10) {
      setUsernameError(`${10 - numeric.length} more digit(s) required`);
    } else {
      setUsernameError('');
    }
  };

  // ── Login handler (async — SHA-256 hashing) ───────────────
  ...
  const onSubmit = (e: React.FormEvent) => {
    ...
    // Frontend length validation (backend validates too)
    if (username.length !== 10 && /^\d+$/.test(username)) {
      setUsernameError('Mobile number must be exactly 10 digits');
      return;
    }
    // Allow non-numeric usernames only for doctor/reception quick-fill
    if (!/^\d{10}$/.test(username) && !/^[a-zA-Z]+$/.test(username)) {
      setUsernameError('Only numeric mobile numbers are allowed (10 digits)');
      return;
    }
  ...
  /**
   * Quick-access for reception and doctor accounts — bypasses numeric validation
   * since these legacy accounts have text-based usernames.
   */
  ...
  // Only show stakeholder doctor accounts (numeric/mobile usernames) — hide legacy text-username doctors
  const doctorUsers = allUsers.filter(u => u.role === 'doctor' && /^\d{10}$/.test(u.username));
      ...
      <div className="relative z-10 w-full max-w-lg animate-fade-in space-y-6">
        ...
        <div className="text-center">
          <div className="inline-flex items-center gap-3 mb-2">
            ...
            <span className="text-2xl font-bold text-white tracking-tight">VPC-HMS</span>
          </div>
        ...
        <div className="glass-card rounded-2xl shadow-2xl p-5 sm:p-8">
          ...
          <form onSubmit={onSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full pl-10 pr-4 py-2.5 rounded-lg border focus:ring-2 outline-none text-base sm:text-sm transition-all bg-white ${
                    usernameError
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-slate-400">Password is case-sensitive</p>
            </div>
        ...
        {/* Quick Access Role Buttons — Reception & Doctors only (no stakeholder shortcuts) */}
        <div>
          <div className="flex items-center justify-between mb-3">
            <p className="text-white/60 text-xs font-medium uppercase tracking-wider">
              Quick Access — Reception &amp; Doctors
            </p>
            <button
              type="button"
            ...
            >
              ...
              {showUserDirectory ? 'Hide Directory' : 'View Directory'}
          ...
          <div className="max-h-[40vh] overflow-y-auto scrollbar-hide grid grid-cols-2 gap-2 pr-0.5">
            ...
            {doctorUsers.map((doc, idx) => (
              <button
                ...
                id={`quick-access-doctor-${doc.username}`}
        ...
        {/* User Directory — shows role and username only, NO passwords */}
        {showUserDirectory && (
          <div className="bg-white/95 backdrop-blur-md rounded-2xl p-4 shadow-2xl border border-white/20 animate-fade-in text-slate-800 space-y-3">
              ...
              <table className="w-full text-xs text-left">
                <thead>
                  <tr className="bg-slate-100 text-slate-600 border-b border-slate-200">
                    ...
                    <th className="px-2.5 py-1.5 font-semibold text-right">Status</th>
                  </tr>
                </thead>
            ...
            <p className="text-[11px] text-slate-400 text-center">
              Stakeholder accounts use mobile number + password. Passwords are not displayed.
            </p>
          </div>
        )}
      </div>
