...
export default function DoctorDashboard() {
  ...
  useEffect(() => {
    if (!selectedPatientId && patients.length > 0) {
      const defaultP = myPatients[0] || patients[0];
      if (defaultP) setSelectedPatientId(defaultP.id);
    }
  }, [patients, myPatients, selectedPatientId]);
  ...
  const handleSaveNotes = () => {
    const targetPatient = patients.find(p => p.id === selectedPatientId) || myPatients[0] || patients[0];
    if (targetPatient && sessionNotes.trim()) {
      saveSessionNote({
        doctorName,
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        const loc: GPSLocation = {
          ...
          patient: targetPatient?.name || stats?.activePatient,
          location: branch,
        };
    ...
    input.onchange = (e) => {
      ...
      if (file) {
        ...
        reader.onload = () => {
          ...
          setUploadSuccess(`${type} for ${targetPatient.name}`);
          ...
          <div className="space-y-4 animate-fade-in">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2 bg-white px-3 py-1.5 rounded-lg border border-slate-200 shadow-sm">
                <label className="text-xs font-medium text-slate-500 whitespace-nowrap">Target Patient:</label>
                <select
                  value={selectedPatientId}
                  onChange={e => setSelectedPatientId(e.target.value)}
                  className="text-xs font-semibold text-slate-700 bg-transparent outline-none cursor-pointer max-w-[200px] truncate"
                >
                  {patients.map(p => (
